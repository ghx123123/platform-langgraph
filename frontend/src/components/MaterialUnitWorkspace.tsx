...
import {
  ...
  GitMerge, Layers3, Link2, Loader2, Maximize, Plus, RefreshCw, Save, Search,
  Send, Sparkles, Trash2, Unlink, X, ZoomIn, ZoomOut,
} from 'lucide-react';
    ...
    {!units.length ? <main className="material-unit-empty"><Database size={30} /><h3>还没有资料单元</h3><p>请先到课程资料库选择进度表、大纲和教材。</p><button type="button" onClick={onGoLibrary}>前往课程资料库</button></main> : <div className="material-unit-layout">
      ...
      <main className="scope-workbench">
        {detailLoading ? <div className="material-unit-loading"><Loader2 className="spin" size={22} /><strong>正在整理讲次、大纲和教材…</strong></div> : <>
          ...
          <section className="planning-step is-open"><header><span>2</span><div><strong>核对相关大纲要求</strong><small>系统只展示与当前讲次相关的目标、重点和难点</small></div><em>{matching ? '匹配中' : alignment ? `${alignment.matches.length}/${alignment.total_candidates} 条` : '等待讲次'}</em></header>{selectedSession && <div className="alignment-content module-scroll">{(() => { if (matching) return <div className="inline-loading"><Loader2 className="spin" size={18} />正在根据“{sessionItem?.title}”分析教学大纲…</div>; if (!alignment?.matches.length) return <p className="step-empty">没有找到可靠匹配。仍可继续选择教材，生成结果会标记“大纲依据缺失”。</p>; const sourceIds = [...new Set(alignment.matches.map((i) => i.evidence?.material_id).filter((v): v is string => Boolean(v)))]; const visible = syllabusFilter === 'all' ? alignment.matches : alignment.matches.filter((i) => !i.evidence?.material_id || i.evidence?.material_id === syllabusFilter); const nameOfS = (mid: string) => unit?.files?.find((f) => f.material_id === mid)?.name || mid.slice(0, 16); return <><div style={{ display: 'flex', gap: 8, alignItems: 'center', margin: '0 0 8px' }}><small style={{ color: '#6b7e93', fontSize: 12 }}>大纲来源</small><select value={syllabusFilter} onChange={(event) => setSyllabusFilter(event.target.value)} style={{ fontSize: 12, padding: '3px 8px', border: '1px solid #cbd8e5', borderRadius: 5, background: '#fff' }}><option value="all">全部大纲（{sourceIds.length} 份）</option>{sourceIds.map((mid) => <option key={mid} value={mid}>{nameOfS(mid)}</option>)}</select><small style={{ color: '#6b7e93', fontSize: 11 }}>导入新大纲后，重新选择讲次即可重新匹配</small></div>{(['objective', 'knowledge', 'key_point', 'difficult_point', 'practice', 'assessment'] as SyllabusRequirementType[]).map((category) => { const items = visible.filter((item) => item.category === category); return items.length ? <div className="requirement-group" key={category}><h4>{requirementLabels[category]}<span>{items.length}</span></h4>{items.map((item) => <label key={item.id} className={selectedRequirements.includes(item.id) ? 'selected' : ''}><input type="checkbox" checked={selectedRequirements.includes(item.id)} onChange={() => toggle(selectedRequirements, item.id, setSelectedRequirements)} /><span><strong>{item.title}</strong><small>{item.content}</small><em>{Math.round(item.score * 100)}% · {item.reason}</em></span></label>)}</div> : null; })}<details className="matching-meta"><summary>查看匹配说明</summary><p>{alignment.model_used ? '已使用智能体语义判断并结合章节、关键词证据。' : '当前使用章节、关键词和文本相似度匹配。'}候选大纲共 {alignment.total_candidates} 条。</p></details></>; })()}</div>}</section>
