...
export function MaterialUnitWorkspace({ refreshKey, onGoLibrary, onCourseDesignCreated }: Props) {
  ...
  // 教师自定义/补充的大纲要求(仅作选中依据, 不生成独立节点); 生成大纲时随 payload 传, 落库到 outline.requirements
  const [customRequirements, setCustomRequirements] = useState<Array<{ category: string; title: string; content: string }>>([]);
  ...
  const selectOutline = useCallback((next: MaterialUnitKnowledgeOutline | null, scopeNow?: import('../types/workflow').MaterialUnitScopeOptions | null) => {
    ...
    if (next) {
      ...
      // 恢复教师自定义/补充的大纲要求(从大纲 requirements 里 custom 项)
      const customs = (next.requirements || [])
        .filter((r) => (r as { custom?: boolean }).custom)
        .map((r) => ({ category: r.category, title: r.title, content: r.content }));
      setCustomRequirements(customs);
    }
  ...
  const runMatch = async (sessionId: string) => {
    ...
    try {
      const result = await materialUnitApi.syllabusMatches(unit.id, [sessionId], true, customRequirements);
      setAlignment(result);
  ...
  const createOutline = async () => {
    ...
    try {
      const created = await materialUnitApi.createKnowledgeOutline(unit.id, {
        ...
        outline_node_ids: selectedTextbook, status: 'draft', custom_requirements: customRequirements,
      });
    ...
    {!units.length ? <main className="material-unit-empty"><Database size={30} /><h3>还没有资料单元</h3><p>请先到课程资料库选择进度表、大纲和教材。</p><button type="button" onClick={onGoLibrary}>前往课程资料库</button></main> : <div className="material-unit-layout">
      ...
      <main className="scope-workbench">
        {detailLoading ? <div className="material-unit-loading"><Loader2 className="spin" size={22} /><strong>正在整理讲次、大纲和教材…</strong></div> : <>
          ...
          <section className="planning-step is-open"><header><span>2</span><div><strong>核对相关大纲要求</strong><small>系统只展示与当前讲次相关的目标、重点和难点；可编辑或补充自定义要求</small></div><em>{matching ? '匹配中' : alignment ? `${alignment.matches.length} 条` : '等待讲次'}</em></header>{selectedSession && <div className="alignment-content module-scroll">{(() => { if (matching) return <div className="inline-loading"><Loader2 className="spin" size={18} />正在根据“{sessionItem?.title}”分析教学大纲…</div>; if (!alignment?.matches.length && !customRequirements.length) return <div className="step-empty-body"><p className="step-empty">没有找到可靠匹配。仍可继续选择教材，生成结果会标记“大纲依据缺失”。</p><div className="custom-req-add"><button type="button" onClick={() => { const title = window.prompt('补充大纲要求标题', ''); if (!title) return; const cat = (window.prompt('分类 (knowledge/objective/key_point/difficult_point/practice/assessment)', 'knowledge') || 'knowledge') as SyllabusRequirementType; const content = window.prompt('要求内容(可空)', '') || ''; setCustomRequirements((cur) => [...cur, { category: cat, title, content }]); }}>＋ 补充要求</button></div></div>; const sourceIds = [...new Set(alignment?.matches.map((i) => i.evidence?.material_id).filter((v): v is string => Boolean(v)) || [])]; const visible = syllabusFilter === 'all' ? (alignment?.matches || []) : (alignment?.matches || []).filter((i) => !i.evidence?.material_id || i.evidence?.material_id === syllabusFilter); const nameOfS = (mid: string) => unit?.files?.find((f) => f.material_id === mid)?.name || mid.slice(0, 16); return (<>
      ...
      {(['objective', 'knowledge', 'key_point', 'difficult_point', 'practice', 'assessment'] as SyllabusRequirementType[]).map((category) => { const items = visible.filter((item) => item.category === category && !item.custom); return items.length ? <div className="requirement-group" key={category}><h4>{requirementLabels[category]}<span>{items.length}</span></h4>{items.map((item) => <label key={item.id} className={selectedRequirements.includes(item.id) ? 'selected' : ''}><input type="checkbox" checked={selectedRequirements.includes(item.id)} onChange={() => toggle(selectedRequirements, item.id, setSelectedRequirements)} /><span><strong>{item.title}</strong><small>{item.content}</small><em>{Math.round(item.score * 100)}% · {item.reason}</em></span></label>)}</div> : null; })}
      {customRequirements.length > 0 && <div className="requirement-group is-custom"><h4>📝 教师自定义/补充<span>{customRequirements.length}</span></h4>{customRequirements.map((c, idx) => <label key={`${c.title}-${idx}`} className="selected" ><input type="checkbox" checked onChange={(e) => { if (!e.target.checked) setCustomRequirements((cur) => cur.filter((_, i) => i !== idx)); }} /><span><strong>{c.title}</strong><small>{c.content}</small><em>教师自定义/补充要求</em></span></label>)}</div>}
      {alignment?.matches?.some((m) => m.custom) && <div className="requirement-group is-custom"><h4>📝 已保存的自定义要求<span>{alignment.matches.filter((m) => m.custom).length}</span></h4>{alignment.matches.filter((m) => m.custom).map((item) => <label key={item.id} className={selectedRequirements.includes(item.id) ? 'selected' : ''}><input type="checkbox" checked={selectedRequirements.includes(item.id)} onChange={() => toggle(selectedRequirements, item.id, setSelectedRequirements)} /><span><strong>{item.title}</strong><small>{item.content}</small><em>教师自定义/补充要求</em></span></label>)}</div>}
      <div className="custom-req-add"><button type="button" onClick={() => { const title = window.prompt('补充大纲要求标题', ''); if (!title) return; const cat = (window.prompt('分类 (knowledge/objective/key_point/difficult_point/practice/assessment)', 'knowledge') || 'knowledge') as SyllabusRequirementType; const content = window.prompt('要求内容(可空)', '') || ''; setCustomRequirements((cur) => [...cur, { category: cat, title, content }]); }}>＋ 补充要求</button></div>
      <details className="matching-meta"><summary>查看匹配说明</summary><p>{alignment?.model_used ? '已使用智能体语义判断并结合章节、关键词证据。' : '当前使用章节、关键词和文本相似度匹配。'}候选大纲共 {alignment?.total_candidates || 0} 条。自定义要求仅作选中依据，生成大纲时不产生独立知识点。</p></details>
    </>); })()}</div>}</section>
