...
import {
  AlertCircle, ArrowDown, ArrowLeft, ArrowUp, BookOpen, BookMarked, Check, CheckCircle2,
  ChevronDown, Database, Edit3, FilePlus2, FileSearch, FileText, Files,
  GitMerge, History, Layers3, Link2, Loader2, Maximize, MessageCircle, Pin, Plus, RefreshCw, Save,
...
import type {
  ...
  MaterialUnitRecord, MaterialUnitScopeAlignment, MaterialUnitScopeOption, MaterialUnitScopeOptions, MaterialUnitSummary,
  MaterialUnitSyllabusMatch,
...
type Dialog =
  ...
  | { mode: 'import-design'; outlineKey: string; materialIds: string[]; primaryMaterialId: string; sessionId: string; sessionTitle: string };
...
// Scope options use a namespaced lecture id while older outlines may contain
// the archive-local id.  Comparing the final identity keeps both formats
// compatible without weakening the import guard.
function scheduleIdentity(value: string | null | undefined): string {
  const raw = String(value || '').trim();
  return raw.startsWith('schedule:') ? raw.slice(raw.lastIndexOf(':') + 1) : raw;
}

/**
 * Scope option ids are regenerated when a document is re-parsed.  A saved
 * outline, however, must remain readable (and auditable) after that happens.
 * Keep the compatibility logic in one pure helper so every restore path uses
 * the same precedence: exact id → source/locator → title/content similarity.
 */
function normalizeRestoreText(value: string | null | undefined): string {
  return String(value || '')
    .normalize('NFKC')
    .toLowerCase()
    .replace(/[\s\u00b7·.,，。；;:：、/\\()[\]{}<>!?！？"“”‘’'\-]+/g, '');
...
function restoreSourceMatches(
  saved: MaterialUnitSyllabusMatch,
  candidate: MaterialUnitScopeOption,
): boolean {
  const evidence = saved.evidence || ({} as MaterialUnitSyllabusMatch['evidence']);
  if (evidence.material_id && candidate.source_material_id !== evidence.material_id) return false;
  if (evidence.source_unit_id && candidate.source_unit_id !== evidence.source_unit_id) return false;
  const savedLocator = normalizeRestoreText(evidence.locator);
  const candidateLocator = normalizeRestoreText(candidate.locator);
  return Boolean(savedLocator && candidateLocator && savedLocator === candidateLocator);
}

function restoreTextSimilarity(saved: MaterialUnitSyllabusMatch, candidate: MaterialUnitScopeOption): number {
  const savedTitle = normalizeRestoreText(saved.title);
  const candidateTitle = normalizeRestoreText(candidate.title);
  const savedContent = normalizeRestoreText(saved.content);
  const candidateContent = normalizeRestoreText(candidate.content);
  if (savedTitle && candidateTitle && savedTitle === candidateTitle) return 1;
  if (savedTitle && candidateTitle && (savedTitle.includes(candidateTitle) || candidateTitle.includes(savedTitle))) return 0.82;
  if (savedContent && candidateContent && (savedContent.includes(candidateContent) || candidateContent.includes(savedContent))) return 0.7;
  const left = `${savedTitle}${savedContent}`;
  const right = `${candidateTitle}${candidateContent}`;
...
function restoreSavedRequirements(
  savedItems: MaterialUnitSyllabusMatch[],
  options: MaterialUnitScopeOptions | null | undefined,
) {
  const candidates = options?.syllabus_items || [];
  const used = new Set<string>();
  const matches: MaterialUnitSyllabusMatch[] = [];
  const selectedIds = new Map<string, string>();

  savedItems.forEach((saved) => {
    let candidate = candidates.find((item) => item.id === saved.id && !used.has(item.id));
    if (!candidate) {
      candidate = candidates.find((item) => !used.has(item.id) && restoreSourceMatches(saved, item));
    }
    if (!candidate) {
      const sameMaterial = candidates
        .filter((item) => !used.has(item.id) && (!saved.evidence?.material_id || item.source_material_id === saved.evidence.material_id))
        .map((item) => ({ item, score: restoreTextSimilarity(saved, item) }))
        .sort((a, b) => b.score - a.score);
      if (sameMaterial[0] && sameMaterial[0].score >= 0.48) candidate = sameMaterial[0].item;
    }

    if (candidate) {
      used.add(candidate.id);
      selectedIds.set(saved.id, candidate.id);
      // Keep the historical score/reason/evidence for auditability while
      // using the current scope id for any new outline submission.
      matches.push({
        ...saved,
        id: candidate.id,
        restored: true,
        original_id: saved.id === candidate.id ? undefined : saved.id,
      });
    } else {
      // Never silently discard a historical requirement.  It is display-only
      // until the teacher runs a fresh match, so stale ids cannot be submitted.
      matches.push({ ...saved, restored: true, stale: true, original_id: saved.id });
    }
  });

  return { matches, selectedIds };
}

function restoreTextbookNodeIds(
  savedIds: string[],
  options: MaterialUnitScopeOptions | null | undefined,
): string[] {
  const candidates = options?.textbook_outline || [];
  const used = new Set<string>();
  const restored: string[] = [];

  savedIds.forEach((savedId) => {
    let candidate = candidates.find((item) => item.id === savedId && !used.has(item.id));
    if (!candidate) {
      const parts = savedId.split(':');
      const materialId = parts.length >= 2 ? parts[parts.length - 2] || '' : '';
      const legacyLocator = parts[parts.length - 1] || '';
      candidate = candidates.find((item) => {
        if (used.has(item.id)) return false;
        if (materialId && item.source_material_id !== materialId) return false;
        const currentLocator = String(item.locator || '').replace(/^section:/, '');
        return Boolean(legacyLocator && currentLocator === legacyLocator);
      });
    }
    if (candidate) {
      used.add(candidate.id);
...
function isForeignScopeItem(
  item: { archive_id?: string | null },
  activeArchiveId: string | null | undefined,
): boolean {
  return Boolean(item.archive_id && activeArchiveId && item.archive_id !== activeArchiveId);
}
...
export function MaterialUnitWorkspace({ refreshKey, onGoLibrary, onCourseDesignCreated }: Props) {
  ...
  const [textbookRestoreWarning, setTextbookRestoreWarning] = useState(0);
  ...
  const [alignmentSource, setAlignmentSource] = useState<'saved' | 'matched' | null>(null);
  ...
  const [matchingWithModel, setMatchingWithModel] = useState(false);
  ...
  const invalidateMatch = useCallback(() => {
    ...
    setMatchingWithModel(false);
    setAlignment(null);
    setAlignmentSource(null);
    setMatchAttempted(false);
    setSelectedRequirements([]);
  }, []);
  useEffect(() => () => { matchAbortRef.current?.abort(); }, []);
  useEffect(() => {
    if (!matching || matchingStartedAt === null) return;
    const tick = () => setMatchingElapsed(Math.max(0, Math.round((Date.now() - matchingStartedAt) / 1000)));
    tick();
    const timer = window.setInterval(tick, 250);
    return () => window.clearInterval(timer);
  }, [matching, matchingStartedAt]);
  // 归一化比较: 空格/标点全去掉
  ...
  const reconcileSelection = useCallback((
    ...
    // Lecture identity can be represented either by the current scoped id
    // (`schedule:<unit>:<schedule>`) or by an older archive-local id. Resolve
    // both forms to the current scope item. If an outline has a non-empty but
    // stale lecture selection, leave it empty so the teacher must choose again
    // instead of silently importing the first unrelated lecture.
    const directSession = sessionIds.find((id) => teachingIds.includes(id));
    const normalizedSession = sessionIds
      .map(scheduleIdentity)
      .map((identity) => (opts.teaching_items || []).find((item) => scheduleIdentity(item.id) === identity)?.id)
      .find((id): id is string => Boolean(id));
    result.session = directSession || normalizedSession || (!sessionIds.length ? teachingIds[0] || '' : '');
    // syllabus: 只保留当前 scope 仍存在的稳定 ID。
    result.reqs = reqIds.filter((id) => syllabusValid.includes(id));
    // Textbook node ids changed from locator-based `section-N` values to
    // stable title hashes. Reconcile legacy selections by material + locator
    // so a restored outline cannot silently fall back to an unrelated book.
    result.tb = restoreTextbookNodeIds(tbIds, opts);
    return result;
  }, []);
  ...
  const selectOutline = useCallback((next: MaterialUnitKnowledgeOutline | null, scopeNow?: import('../types/workflow').MaterialUnitScopeOptions | null) => {
    ...
    if (next) {
      ...
      setSelectedSession(reconciled.session);
      setSelectedTextbook(reconciled.tb);
      setTextbookRestoreWarning(Math.max(0, next.selected_textbook_node_ids.length - reconciled.tb.length));
      setMatchError('');

      // 大纲版本本身已经保存了匹配结果。恢复时把它作为只读快照展示，
      // 不重新调用模型，避免刷新页面产生额外请求或把历史结果误标为实时。
      const savedRequirements = restoreSavedRequirements(next.requirements || [], scopeNow ?? scope);
      const selectedIds = new Set(reconciled.reqs);
      // Older outlines sometimes persisted requirements but left the explicit
      // selection list empty.  In that case retain the saved recommendations;
      // otherwise map every historical selection to its current scope id.
      (next.selected_syllabus_item_ids || []).forEach((id) => {
        const mapped = savedRequirements.selectedIds.get(id);
        if (mapped) selectedIds.add(mapped);
        else if ((scopeNow ?? scope)?.syllabus_items.some((item) => item.id === id)) selectedIds.add(id);
      });
      if (!(next.selected_syllabus_item_ids || []).length) {
        savedRequirements.matches.forEach((item) => {
          if (!item.stale && item.recommended) selectedIds.add(item.id);
        });
      }
      setSelectedRequirements([...selectedIds]);
      const savedMatches = savedRequirements.matches;
      if (savedMatches.length) {
        const restoredSession = reconciled.session;
        const restoredTeaching = (scopeNow ?? scope)?.teaching_items?.filter(
          (item) => item.id === restoredSession || scheduleIdentity(item.id) === scheduleIdentity(restoredSession),
        ) || [];
        setAlignment({
          unit_id: (scopeNow ?? scope)?.unit_id || next.unit_id,
          teaching_items: restoredTeaching,
          matches: savedMatches,
          total_candidates: Math.max(savedMatches.length, (scopeNow ?? scope)?.syllabus_items?.length || 0),
          matching_method: 'deterministic',
          model_used: false,
        });
        setAlignmentSource('saved');
    ...
    else setTextbookRestoreWarning(0);
  ...
  const openUnit = useCallback(async (unitId: string) => {
    ...
    try {
      ...
      if (!latest) {
        setSelectedSession(''); setSelectedRequirements([]); setSelectedTextbook([]); setTextbookRestoreWarning(0);
      }
    } catch (reason) { setError(getErrorMessage(reason)); }
    finally { setDetailLoading(false); }
  }, [invalidateMatch, selectOutline]);
  ...
  const runMatch = async (sessionId: string, useModel = false) => {
    if (!unit || !sessionId) return;
    const requestId = ++matchRequestRef.current;
    matchAbortRef.current?.abort();
    const controller = new AbortController();
    matchAbortRef.current = controller;
    setSelectedSession(sessionId); setMatching(true); setMatchingWithModel(useModel); setMatchingStartedAt(Date.now()); setMatchingElapsed(0); setMatchError(''); setError(''); setSelectedRequirements([]); setAlignment(null);
    setAlignmentSource(null); setMatchAttempted(true);
    try {
      const result = await materialUnitApi.syllabusMatches(unit.id, [sessionId], useModel, controller.signal);
      if (requestId !== matchRequestRef.current || controller.signal.aborted) return;
      setAlignment(result);
      setAlignmentSource('matched');
      setSelectedRequirements(result.matches.filter((item) => item.recommended).map((item) => item.id));
    } catch (reason) {
      if (controller.signal.aborted || requestId !== matchRequestRef.current) return;
      const message = getErrorMessage(reason);
      setMatchError(message); setError(message); setAlignment(null);
    } finally {
      if (requestId === matchRequestRef.current) {
        setMatching(false); setMatchingStartedAt(null); matchAbortRef.current = null;
      }
    }
  };
  ...
  const importToDesign = async () => {
    ...
    if (selectedOutline.unit_id !== unit.id) {
      setError('当前知识大纲不属于所选资料单元，请重新选择。');
      return;
    }
    // Keep the lecture snapshot captured when the dialog opened. This prevents
    // a late response or a user switching the lecture from importing a
    // different lecture's outline into the design.
    if (selectedOutline.selected_session_ids.length > 0 && !selectedOutline.selected_session_ids.some((id) => scheduleIdentity(id) === scheduleIdentity(dialog.sessionId))) {
      setError(`知识大纲“${selectedOutline.title}”对应${selectedOutline.selected_session_ids.join('、')}，与当前导入讲次“${dialog.sessionTitle}”不一致。请切换到匹配版本后再导入。`);
      return;
    }
    const availableMaterialIds = new Set(designImportFiles.map((file) => file.material_id));
    if (dialog.materialIds.some((id) => !availableMaterialIds.has(id))) {
      setError('导入资料已发生变化，请刷新资料单元后重新选择。');
      return;
    }
    ...
    try {
      ...
      const design = await courseDesignApi.create({
        archive_id: unit.archive_id, schedule_id: scheduleIdentity(dialog.sessionId) || null, material_ids: dialog.materialIds, primary_material_id: dialog.primaryMaterialId,
  ...
  const allowedScopeUnitIds = useMemo(() => new Set([
    unit?.id,
    ...(unit?.linked_units || [])
      .filter((linked) => linked.archive_id === unit?.archive_id)
      .map((linked) => linked.unit_id),
  ].filter((value): value is string => Boolean(value))), [unit]);
  const isForeignEvidence = (evidence: { source_unit_id?: string | null } | null | undefined) =>
    Boolean(evidence?.source_unit_id && !allowedScopeUnitIds.has(evidence.source_unit_id));
  ...
  return <div className="material-unit-workspace">
    ...
    {!units.length ? <main className="material-unit-empty"><Database size={30} /><h3>还没有资料单元</h3><p>请先到课程资料库选择进度表、大纲和教材。</p><button type="button" onClick={onGoLibrary}>前往课程资料库</button></main> : <div className="material-unit-layout" style={muLayoutStyle}>
      ...
      <main className="scope-workbench">
        ...
        <section className="planning-step is-open"><header><span>1</span><div><strong>选择本次课</strong><small>从进度表中选择一条课程安排</small></div><em>{selectedSession ? '已选择' : '待选择'}</em></header><div className="session-options module-scroll">{(() => { const all = scope?.teaching_items || []; const scheduleIds = [...new Set(all.map((i) => i.source_material_id))]; const filtered = scheduleFilter === 'all' ? all : all.filter((i) => i.source_material_id === scheduleFilter); if (!all.length) return <p className="step-empty">尚未识别到进度表，请关联或导入进度表文件。</p>; return <><div style={{ display: 'flex', gap: 8, alignItems: 'center', margin: '0 0 8px' }}><small style={{ color: '#6b7e93', fontSize: 12 }}>进度表来源</small><select value={scheduleFilter} onChange={(event) => { invalidateMatch(); setScheduleFilter(event.target.value); setSelectedSession(''); setMatchError(''); }} style={{ fontSize: 12, padding: '3px 8px', border: '1px solid #cbd8e5', borderRadius: 5, background: '#fff' }}><option value="all">全部进度表（{scheduleIds.length} 份）</option>{scheduleIds.map((mid) => <option key={mid} value={mid}>{all.find((i) => i.source_material_id === mid)?.source_name || mid.slice(0, 8)}</option>)}</select><small style={{ color: '#6b7e93', fontSize: 11 }}>切换来源后需重新选择讲次（大纲匹配随之更新）</small></div>{filtered.map((item) => { const crossArchive = isForeignScopeItem(item, unit?.archive_id); return <button type="button" key={item.id} disabled={crossArchive} aria-disabled={crossArchive} className={`${selectedSession === item.id ? 'selected' : ''} ${crossArchive ? 'is-cross-archive' : ''}`} onClick={() => { if (!crossArchive) void runMatch(item.id); }}><span className="radio-mark">{selectedSession === item.id && <Check size={13} />}</span><span><strong>{item.title}</strong><small>{item.content}</small><em>{crossArchive ? `来源：${item.source_name} · 跨库仅浏览` : `来源：${item.source_name}`}</em></span></button>; })}</>; })()}</div></section>

          <section className="planning-step is-open"><header><span>2</span><div><strong>核对相关大纲要求</strong><small>系统只展示与当前讲次相关的目标、重点和难点</small></div><em>{matching ? '匹配中' : alignment ? `${alignment.matches.length} 条${alignmentSource === 'saved' ? ' · 已恢复' : ''}` : selectedSession && matchAttempted ? '已匹配' : '等待讲次'}</em></header>{selectedSession && <div className="alignment-content module-scroll">{(() => { if (matching) return <div className="inline-loading"><Loader2 className="spin" size={18} />正在根据“{sessionItem?.title}”分析教学大纲…</div>; if (!matchAttempted) return <p className="step-empty">选择讲次后，系统会自动匹配相关大纲要求；也可以先浏览并选择教材范围。</p>; if (!alignment?.matches.length) return <p className="step-empty">没有找到可靠匹配。仍可继续选择教材，生成结果会标记“大纲依据缺失”。</p>; const sourceIds = [...new Set(alignment.matches.map((i) => i.evidence?.material_id).filter((v): v is string => Boolean(v)))]; const visible = syllabusFilter === 'all' ? alignment.matches : alignment.matches.filter((i) => !i.evidence?.material_id || i.evidence?.material_id === syllabusFilter); const nameOfS = (mid: string) => unit?.files?.find((f) => f.material_id === mid)?.name || mid.slice(0, 16); return (<>
      {alignmentSource === 'saved' && <div className="alignment-restored" role="status"><CheckCircle2 size={14} /><span>已恢复知识大纲 v{outline?.version} 的匹配结果；切换讲次后会重新匹配。</span>{alignment.matches.some((item) => item.stale) && <em>{alignment.matches.filter((item) => item.stale).length} 条历史要求暂未对齐</em>}</div>}
    ...
    {insertPicker && <div className="unit-dialog-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && setInsertPicker(null)}><section className="gpi-dialog" role="dialog" aria-modal="true"><header className="gpi-header"><div className="gpi-header-title"><span className="gpi-app-icon"><BookOpen size={20} /></span><div><strong>图谱导入知识大纲</strong><span className="gpi-breadcrumb">{insertPicker.title} · 图谱节点关系与问答</span></div></div><div className="gpi-header-actions"><button type="button" className="gpi-btn gpi-btn-primary" disabled={mutating} onClick={() => { void importNodes(insertPicker.nodes.map((n) => n.id), insertPicker.nodeId); }}><Layers3 size={15} />一键导入全部（{insertPicker.nodes.length}）</button><button type="button" className="gpi-close" onClick={() => setInsertPicker(null)} aria-label="关闭"><X size={17} /></button></div></header>
      ...
      <div className="gpi-canvas-toolbar"><button type="button" className="gpi-icon-btn-on" aria-label="缩小" title="缩小" onClick={() => setGraphZoom((z) => Math.max(0.5, z - 0.25))}><ZoomOut size={13} /></button><span className="gpi-zoomval">{Math.round(graphZoom * 100)}%</span><button type="button" className="gpi-icon-btn-on" aria-label="放大" title="放大" onClick={() => setGraphZoom((z) => Math.min(2.5, z + 0.25))}><ZoomIn size={13} /></button><span className="gpi-toolbar-sep" /><button type="button" className="gpi-icon-btn" aria-label="适应画布" title="适应画布" onClick={() => setGraphZoomToken((v) => v + 1)}><Maximize size={13} /></button></div>
    </>); })()}
    </section></div>}
  </div>;
}
...
function UnitDialog({ dialog, units, files, outlines, mutating, onChange, onClose, onSubmit }: { dialog: Dialog; units: MaterialUnitSummary[]; files: Array<MaterialUnitFileAnalysis & { sourceLabel: string }>; outlines: MaterialUnitKnowledgeOutline[]; mutating: boolean; onChange: (dialog: Dialog) => void; onClose: () => void; onSubmit: () => void }) {
  ...
  const importSessionMismatch = dialog.mode === 'import-design' && Boolean(selectedImportOutline)
    && (selectedImportOutline?.selected_session_ids.length || 0) > 0
    && !selectedImportOutline?.selected_session_ids.some((id) => scheduleIdentity(id) === scheduleIdentity(dialog.sessionId));
  ...
  return <div className="unit-dialog-backdrop"><section className={`unit-dialog ${dialog.mode === 'import-design' ? 'is-wide' : ''}`} role="dialog" aria-modal="true"><header><div>{dialog.mode === 'merge' ? <GitMerge size={19} /> : dialog.mode === 'link-files' ? <Link2 size={19} /> : dialog.mode === 'delete' || dialog.mode === 'delete-outline' ? <Trash2 size={19} /> : dialog.mode === 'import-design' ? <Send size={19} /> : <Edit3 size={19} />}<span><strong>{dialog.mode === 'merge' ? '整合为一个单元' : dialog.mode === 'link-files' ? '关联其他单元资料' : dialog.mode === 'delete' ? '删除资料单元' : dialog.mode === 'delete-outline' ? '删除知识大纲' : dialog.mode === 'import-design' ? '确认导入课程设计' : '重命名资料单元'}</strong><small>{dialog.mode === 'merge' ? '来源单元会从列表移除，课程资料库原文件不会删除' : dialog.mode === 'link-files' ? '只建立文件引用，来源单元和原文件保持不变' : dialog.mode === 'delete' ? '只删除资料单元记录，不删除课程资料库原文件' : dialog.mode === 'delete-outline' ? '删除操作不会影响教材、进度表和教学大纲原文件' : dialog.mode === 'import-design' ? '请核对知识大纲版本、主教材和支撑资料；未解析材料会在导入前自动补齐解析' : '资料关系和分析结果保持不变'}</small></span></div><button type="button" onClick={onClose} aria-label="关闭"><X size={17} /></button></header>
    ...
    {dialog.mode === 'import-design' && (importSessionMismatch ? <div className="import-session-warning" role="alert"><AlertCircle size={15} /><span>当前导入讲次：<b>{dialog.sessionTitle}</b>。所选大纲绑定了其他讲次，已禁止导入。</span></div> : <div className="import-session-ok"><CheckCircle2 size={14} />已锁定讲次：{dialog.sessionTitle}</div>)}
    </div>
    <footer><button type="button" onClick={onClose}>取消</button><button type="button" className={dialog.mode === 'delete' || dialog.mode === 'delete-outline' ? 'danger' : 'primary'} disabled={mutating || (dialog.mode === 'rename' && !dialog.title.trim()) || (dialog.mode === 'merge' && !dialog.sourceIds.length) || (dialog.mode === 'link-files' && (!dialog.sourceUnitId || !dialog.materialIds.length)) || (dialog.mode === 'import-design' && (importSessionMismatch || !dialog.outlineKey || !dialog.materialIds.length || !dialog.primaryMaterialId || primaryImportFile?.category !== 'textbook'))} onClick={onSubmit}>{mutating ? <Loader2 className="spin" size={15} /> : <Check size={15} />}{dialog.mode === 'merge' ? '确认整合' : dialog.mode === 'link-files' ? '确认关联' : dialog.mode === 'delete' || dialog.mode === 'delete-outline' ? '确认删除' : dialog.mode === 'import-design' ? '确认并进入课程设计' : '保存名称'}</button></footer>
  </section></div>;
}
    ...
    <div style={L.body}>
      <section style={{ ...L.col, minWidth: 340 }}>
        ...
        <div style={L.graphBox}>
          <div style={L.graphHead}><BookMarked size={15} aria-hidden="true" />阅读图谱 <small style={{ color: '#8190a0', fontWeight: 400 }}>教材研读 · dsh</small></div>
          ...
          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' as const }}>
            ...
            <button type="button" onClick={() => void saveGraphNote()} disabled={graphBusy || !rounds.length} style={L.graphBtn}><Save size={14} aria-hidden="true" />保存为图谱节点{contextNodeId ? '(子节点)' : ''}</button>
          </div>
          {graphError && <div style={{ color: '#b3352c', fontSize: 11 }}>{graphError}</div>}
          {graphNodes.length > 0 && <details style={{ borderTop: '1px solid #e3e9f1', paddingTop: 6 }}><summary style={{ fontSize: 11.5, color: '#52677f', cursor: 'pointer' }}>已保存图谱节点（{graphNodes.length}）</summary><div style={{ display: 'grid', gap: 6, marginTop: 6 }}>{[...graphNodes].sort((a, b) => (a.parent_id ? 1 : 0) - (b.parent_id ? 1 : 0)).map((node) => <div key={node.id} style={{ border: '1px solid #dbe4ee', borderRadius: 5, padding: 6, background: '#fff', marginLeft: node.parent_id ? 12 : 0 }}><div style={{ display: 'flex', alignItems: 'center', gap: 5 }}><b style={{ fontSize: 11.5, color: '#1f4e8c', flex: 1 }}>{node.parent_id ? '└ ' : ''}{node.title}</b>{node.section_title && <small style={{ color: '#8190a0', fontSize: 10 }}>{node.section_title.slice(0, 18)}</small>}<button type="button" aria-label="基于此讨论" title="基于此讨论" onClick={() => { setContextNodeId(node.id); setRoundsTitle(`基于「${node.title}」`); setQuote(''); setQuestion(''); setChatId(''); setRounds([]); }} style={{ ...L.graphBtn, minWidth: 22, padding: '0 5px' }}><MessageCircle size={14} aria-hidden="true" /></button><button type="button" aria-label={`删除图谱节点 ${node.title}`} title="删除" onClick={() => { if (window.confirm(`删除图谱节点「${node.title}」及子节点？`)) void materialUnitApi.graphNodeDelete(unitId!, node.id).then(() => void loadNodes()).catch((reason) => setGraphError(getErrorMessage(reason))); }} style={{ ...L.graphBtn2, minWidth: 20, padding: '0 5px', color: '#b3352c' }}><X size={12} aria-hidden="true" /></button></div><pre style={{ margin: '4px 0 0', fontSize: 10.5, color: '#52677f', whiteSpace: 'pre-wrap', maxHeight: 110, overflow: 'auto' }}>{(node.content || '').replace(/^```(?:json|md|markdown)?\s*/m, '').replace(/\s*```\s*$/m, '').slice(0, 400)}</pre>{(() => { const imps = graphImports[node.id] || []; return imps.length ? <div style={{ marginTop: 5, borderTop: '1px solid #edf1f5', paddingTop: 5 }}><small style={{ color: '#6b7e93', fontSize: 10.5, display: 'block', marginBottom: 4 }}><Pin size={12} aria-hidden="true" />已导入到知识大纲（{imps.length} 处）</small>{imps.map((imp, ii) => <div key={ii} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '2px 0' }}><span style={{ flex: 1, fontSize: 10.5, color: '#40546d', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>v{imp.outline_version} · {imp.node_title}</span><button type="button" aria-label={`取消导入 ${imp.node_title}`} title={`取消导入 （${imp.node_title}）`} onClick={() => { if (window.confirm(`取消导入「${imp.node_title}」的这份教材研读补充？`)) void materialUnitApi.graphNodeUnlinkOutline(unitId!, node.id, { outline_id: imp.outline_id, node_id: imp.node_id }).then(() => void loadNodes()).catch((reason) => setGraphError(getErrorMessage(reason))); }} style={{ ...L.graphBtn2, minWidth: 20, padding: '0 5px', color: '#b3352c' }}><Unlink size={11} aria-hidden="true" /></button></div>)}</div> : <small style={{ color: '#8190a0', fontSize: 10.5 }}>在右侧知识大纲点击「教材研读」挂载到对应章节</small>; })()}</div>)}</div></details>}
        </div>
      </section>
