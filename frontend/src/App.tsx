...
const phases: Array<{
  ...
  {
    key: "teach_knowledge",
    phase: "teach_knowledge",
    name: "教师讲授",
    short: "课堂实施",
  },
  {
    key: "student_question",
    phase: "student_question",
    name: "学生提问",
    short: "分层学习者",
  },
  {
    key: "teacher_answer",
    phase: "teacher_answer",
    name: "教师答疑",
    short: "澄清误区",
  },
  {
...
function materialSubpageFromPath(pathname: string): MaterialSubpage {
  return pathname.startsWith("/materials/preparation")
    ? "preparation"
    : "units";
}

const stagePaths: Record<AppStage, string> = {
  overview: "/overview",
  hub: "/hub",
  materials: "/materials",
  design: "/design",
  exports: "/exports/compose",
};

const archiveExcludedSegments = new Set([
  "node_modules",
  "dist",
  "build",
  ".git",
    ...
    (file) =>
      !archivePath(file)
        ...
        .some(
          (part) =>
            part.startsWith(".") ||
            archiveExcludedSegments.has(part.toLowerCase()),
        ),
  );
}
...
function designSummary(design: CourseDesignRecord): CourseDesignSummary {
  ...
  return {
    ...
    latest_export_at: exports.length
      ? exports[exports.length - 1].created_at
      : null,
...
function visualEvidenceContext(items: DocumentVisualAnalysis[]): string {
  ...
  const pages = items.map((item) => {
    const elements = item.visual_elements
      .map((element) => `${element.title}：${element.description}`)
      .join("；");
    const corrections = item.ocr_corrections
      .map(
        (correction) =>
          `${correction.recognized || "漏字"}→${correction.corrected}`,
      )
      .join("；");
    const notes = item.teaching_notes.join("；");
    return [
      `第${item.page_number}页：${item.summary}`,
      elements && `视觉要素：${elements}`,
      corrections && `文字复核：${corrections}`,
      notes && `备课提示：${notes}`,
    ]
      .filter(Boolean)
      .join("\n");
  ...
  events.forEach((event) => {
    ...
    if (payload.analysis)
      data.content_analysis =
        payload.analysis as TeachingData["content_analysis"];
    if (payload.knowledge_points)
      data.knowledge_points =
        payload.knowledge_points as TeachingData["knowledge_points"];
    if (payload.document_sections)
      data.document_sections =
        payload.document_sections as TeachingData["document_sections"];
    if (payload.framework)
      data.teaching_framework =
        payload.framework as TeachingData["teaching_framework"];
    if (payload.messages) {
    ...
    if (typeof payload.iteration === "number")
      data.current_iteration = payload.iteration;
  });
  return data;
}
...
function InterventionPanel({
  ...
  return (
    <div className="intervene-card">
      <header>
        <Hand size={15} />
        <strong>{design ? "教学设计待确认" : "这一轮由谁来答疑"}</strong>
        {pending.iteration > 0 && (
          <span className="round-tag">第 {pending.iteration} 轮</span>
        )}
      </header>
      <p className="intervene-prompt">{pending.prompt}</p>
      {design && !!pending.context.stages?.length && (
        <ul className="intervene-stages">
          {pending.context.stages.map((stage) => (
            <li key={stage.name}>
              <strong>{stage.name}</strong>
              <span>{stage.activity}</span>
              <em>{stage.minutes} 分钟</em>
            </li>
          ))}
        </ul>
      )}
      {!design && !!pending.context.questions?.length && (
        <ul className="intervene-questions">
          {pending.context.questions.map((question, index) => (
            <li key={index}>
              <span className={`level level-${question.level || "medium"}`}>
                {question.level ? levelNames[question.level] : "学生"}
              </span>
      ...
      <textarea
        ...
        placeholder={
          design
            ? "写下你对重难点或课堂环节的修改意见，例如：难点环节延长到 15 分钟，增加对比练习"
            : "写下完整答复，或只写要点交给教师扩写"
        }
      />
        ...
        {design ? (
          <>
            <button
              ...
              title={empty ? "请先写下修改意见" : "按我的意见重新设计"}
            >
              {busy ? (
          ...
          <>
            <button
              ...
              title={empty ? "请先写下答复内容" : "这段内容将作为本轮答疑"}
            >
              {busy ? (
                <Loader2 className="spin" size={15} />
              ) : (
...
function groupByIteration(messages: TeachingMessage[]) {
  ...
  messages.forEach((message) => {
    const existing = groups.find(
      (group) => group.iteration === message.iteration,
    );
...
function App() {
  // Legacy process helpers remain available for the material/result workspace migration.
  void phases;
  void InterventionPanel;
  void groupByIteration;
  ...
  const userSelectedRunRef = useRef<string | null>(
    window.localStorage.getItem("platform.classroom.run_id"),
  );
  ...
  // React state is committed after the event handler returns.  A very fast
  // double click can therefore enter createSession twice before `disabled`
  // reaches the DOM.  This synchronous guard makes one click gesture equal
  // exactly one workflow Run; backend run-scoped locks remain the final
  // idempotency boundary for preparation.
  ...
  const [appStage, setAppStage] = useState<AppStage>(() =>
    stageFromPath(window.location.pathname),
  );
  const [projectPanelOpen, setProjectPanelOpen] = useState(true);
  const [detailPanelOpen, setDetailPanelOpen] = useState(false);
  const [messageView, setMessageView] = useState<"latest" | "all" | number>(
    "latest",
  );
  ...
  const [scope, setScope] = useState<TeachingScope>({
    selected_point_titles: [],
    estimated_minutes: 45,
    depth: "standard",
  });
  const [pagesCustomized, setPagesCustomized] = useState(false);
  const [scopeConfirmed, setScopeConfirmed] = useState(false);
  const [interventions, setInterventions] = useState<InterventionPoint>({
    after_design: false,
  ...
  const [selectedArchive, setSelectedArchive] =
    useState<CourseArchiveDetail | null>(null);
  ...
  const [archiveImportCounts, setArchiveImportCounts] = useState({
    total: 0,
    content: 0,
  });
  const [designs, setDesigns] = useState<CourseDesignSummary[]>([]);
  const [activeDesign, setActiveDesign] = useState<CourseDesignRecord | null>(
    null,
  );
  const [designLoading, setDesignLoading] = useState(false);
  const [exportSubpage, setExportSubpage] = useState<ExportSubpage>(() =>
    exportSubpageFromPath(window.location.pathname),
  );
  const [materialSubpage, setMaterialSubpage] = useState<MaterialSubpage>(() =>
    materialSubpageFromPath(window.location.pathname),
  );
  ...
  const navigateMaterials = useCallback(
    (subpage: MaterialSubpage, replace = false) => {
      setMaterialSubpage(subpage);
      setAppStage("materials");
      const path =
        subpage === "preparation" ? "/materials/preparation" : "/materials";
      if (window.location.pathname !== path)
        window.history[replace ? "replaceState" : "pushState"](
          { stage: "materials", subpage },
          "",
          path,
        );
    },
  ...
  const navigateExport = useCallback(
    (subpage: ExportSubpage, replace = false) => {
      setExportSubpage(subpage);
      setAppStage("exports");
      const path = `/exports/${subpage}`;
      if (window.location.pathname !== path)
        window.history[replace ? "replaceState" : "pushState"](
          { stage: "exports", subpage },
          "",
          path,
        );
    },
  ...
  useEffect(() => {
    if (selectedRunId)
      window.localStorage.setItem("platform.classroom.run_id", selectedRunId);
    else window.localStorage.removeItem("platform.classroom.run_id");
  }, [selectedRunId]);
  ...
  const refresh = useCallback(async () => {
    ...
    setSelectedRunId((value) =>
      value && response.items.some((item) => item.id === value)
        ? value
  ...
  useEffect(() => {
    Promise.all([refresh(), refreshModel()])
  ...
  const loadArchive = useCallback(async (archiveId: string) => {
    ...
    setError(null);
    try {
      setSelectedArchive(await courseArchiveApi.get(archiveId));
    } catch (reason) {
      setError(getErrorMessage(reason));
    } finally {
  ...
  const refreshArchives = useCallback(async () => {
    ...
    if (response.items.length && !selectedArchive)
      await loadArchive(response.items[0].id);
  ...
  const loadDesign = useCallback(async (designId: string) => {
    ...
    setError(null);
    try {
      setActiveDesign(await courseDesignApi.get(designId));
    } catch (reason) {
      setError(getErrorMessage(reason));
    } finally {
  ...
  const refreshDesigns = useCallback(async () => {
    ...
    if (response.items.length && !activeDesign)
      await loadDesign(response.items[0].id);
  ...
  useEffect(() => {
    ...
    if (
      ...
      !activeDesign.run_id &&
      selectedRunId &&
      userSelectedRunRef.current !== selectedRunId
    )
  ...
  useEffect(() => {
    if (
      !activeDesign?.knowledge_outline_id ||
      activeDesign.run_id ||
      parsedDoc ||
      ...
      return;
    hydratedDesignId.current = activeDesign.id;
    void courseArchiveApi
      .prepare(activeDesign.archive_id, {
      ...
      .then((pack) => {
        setParsedDoc({
          ...
          knowledge_points: activeDesign.content.knowledge_points.map(
            (point) => ({
              title: point,
              chapter: activeDesign.content.chapter,
              is_key_point: activeDesign.content.key_points.includes(point),
              difficulty_level: activeDesign.content.difficult_points.includes(
                point,
              )
                ? "困难"
                : "标准",
        ...
        setContext(
          `已导入资料单元知识大纲 v${activeDesign.knowledge_outline_version || 1}。${activeDesign.content.session_label || ""}`.trim(),
        );
        if (!userSelectedRunRef.current) setWorkspaceView("material");
  ...
  useEffect(() => {
    ...
    setArchiveElapsed(0);
    const timer = window.setInterval(
      () => setArchiveElapsed(Math.floor((Date.now() - startedAt) / 1000)),
      1000,
    );
    return () => window.clearInterval(timer);
  }, [archiveImporting]);
  ...
  const legacyWorkflowRunId =
    selectedRun?.teaching_data?.workflow_version === "classroom_v2"
      ? null
      : selectedRunId;
  const { events, connection } = useWorkflowEvents(
    legacyWorkflowRunId,
    selectedRun?.status,
    onTerminal,
  );
  ...
  const submitResume = async (action: ResumeInput["action"]) => {
    ...
    setError(null);
    try {
      await workflowApi.resumeRun(selectedRun.id, {
        action,
  ...
  const previewDocument = useMemo(() => {
    if (parsedDoc)
      return {
        ...
        analysis: undefined,
        defaultView: (activeDesign?.knowledge_outline_id
          ? "text"
          : "original") as "text" | "original",
        knowledge_outline_nodes: (activeDesign?.source_snapshot
          ?.knowledge_nodes ||
          []) as unknown as import("./types/workflow").MaterialUnitKnowledgeNode[],
        knowledge_outline_version: activeDesign?.knowledge_outline_version,
        onVisualEvidenceChange: setVisualEvidence,
      };
    if (selectedRun && teaching.document_text)
      return {
  ...
  const importArchiveFolder = async (
    inputFiles: File[],
    navigateAfter = true,
  ...
  ): Promise<{ archiveId: string; sourceId: string } | null> => {
    const allFiles = relevantArchiveFiles(inputFiles);
    if (!allFiles.length && !sourceName?.trim()) {
      setError("没有读取到可建立索引的文件或文件夹。");
      return null;
    }
    const archiveName =
      sourceName?.trim() ||
      archivePath(allFiles[0]).split("/")[0] ||
      "学期资料库";
    ...
    setError(null);
    if (navigateAfter) navigateStage("materials");
    try {
      const selectedTarget = archives.find(
        (item) => item.id === targetArchiveId,
      );
      const relativePath = (file: File) => {
        const parts = archivePath(file)
          .replace(/\\/g, "/")
          .split("/")
          .filter(Boolean);
      ...
      const registered = await dataHubApi.registerBrowserSource({
        ...
        selection_kind:
          ...
          allFiles.some((file) => !!file.webkitRelativePath)
            ? "folder"
            : "files",
        manifest: allFiles.map((file) => ({
      ...
      allFiles.forEach((file) =>
        indexedSourceFiles.current.set(
          `${cachePrefix}${relativePath(file).toLocaleLowerCase()}`,
          file,
        ),
      );
  ...
  const prepareArchive = async (input: PrepareArchiveInput) => {
    ...
    setArchiveImportCounts({ total: 0, content: 0 });
    setError(null);
    try {
      const selected = selectedArchive.materials.filter((item) =>
        input.material_ids.includes(item.id),
      );
      const pendingBrowserFiles = selected.filter(
        (item) => !item.document_id && item.source_kind === "upload",
      );
      const pendingUploads = pendingBrowserFiles.map((item) => ({
        item,
        file:
          item.source_folder_id && item.source_relative_path
            ? indexedSourceFiles.current.get(
                `${selectedArchive.id}:${item.source_folder_id}:${item.source_relative_path.toLocaleLowerCase()}`,
              )
            : undefined,
      }));
      ...
      if (unavailable.length) {
        const examples = unavailable
          ...
          .join("、");
        setError(
          `有 ${unavailable.length} 份已索引资料需要重新授权原件（${examples}${unavailable.length > 3 ? "等" : ""}）。请返回数据中台，点击对应一级来源的刷新图标并重新选择原文件或文件夹。`,
        );
        return;
      }
      ...
      for (const [sourceId, entries] of groups) {
        ...
        const flush = async () => {
          ...
          await dataHubApi.uploadSourceFiles(
            selectedArchive.id,
            sourceId,
            batch,
          );
        ...
        for (const entry of entries) {
          if (
            batch.length >= 100 ||
            (batch.length > 0 &&
              batchBytes + entry.file.size > 72 * 1024 * 1024)
          )
            await flush();
          batch.push(entry);
      ...
      const extracted = await courseArchiveApi.extract(
        selectedArchive.id,
        input.material_ids,
      );
      setSelectedArchive(extracted);
      const [pack, design]: [PreparationPack, CourseDesignRecord] =
        await Promise.all([
  ...
  const removeArchive = async (
    archiveId: string,
    confirmed = false,
  ): Promise<ArchiveDeletionResult | null> => {
    if (
      !confirmed &&
      !window.confirm(
        "删除当前课程资料库及其平台副本、教学设计、资料包和关联会话？本机源目录不会被删除。",
      )
    )
      return null;
    setError(null);
    ...
    try {
      const [archiveResponse, designResponse, runResponse] = await Promise.all([
        courseArchiveApi.list(),
        courseDesignApi.list(),
      ...
      setSelectedRunId((current) =>
        runResponse.items.some((item) => item.id === current)
          ? current
          : runResponse.items[0]?.id || null,
      );
      if (activeDesign?.archive_id === archiveId) {
        setActiveDesign(null);
      }
      if (archiveResponse.items.length)
        await loadArchive(archiveResponse.items[0].id);
      else setSelectedArchive(null);
    } catch (reason) {
      setError(`课程已删除，但列表刷新失败：${getErrorMessage(reason)}`);
    }
    return result;
  };
  const createSession = async () => {
    if (createSessionInFlightRef.current) return;
    if (!parsedDoc || title.trim().length < 2) {
      setError("请先上传课程文档并填写课程标题。");
      return;
    }
    if (scope.selected_point_titles.length === 0) {
      setError("请至少选择一个需要打磨的知识点。");
      return;
    }
    if (!scopeConfirmed) {
      setError("请先核对知识点、课时和讲解深度，并确认本次范围。");
      return;
    }
    ...
    try {
      ...
      const visualContext = visualEvidenceContext(visualEvidence).slice(
        0,
        2000,
      );
      const enrichedContext = [baseContext, visualContext]
        .filter(Boolean)
        .join("\n\n");
      const run = await workflowApi.createRun({
        title: title.trim(),
        archive_id: activeDesign?.archive_id,
        design_id: activeDesign?.id,
        document_id: parsedDoc.document_id,
        document_name: parsedDoc.file_name,
        document_text: parsedDoc.raw_text,
        document_sections: parsedDoc.sections || [],
        extraction_report: parsedDoc.extraction_report,
        knowledge_points: editedPoints,
        ...
        scope: {
          ...scope,
          ppt_slide_count: scope.ppt_slide_count || scope.estimated_minutes,
        },
      });
      setRuns((current) => [run, ...current]);
      setSelectedRunId(run.id);
      userSelectedRunRef.current = run.id;
      window.localStorage.setItem("platform.classroom.run_id", run.id);
      setParsedDoc(null);
      setTitle("");
      setContext("");
      ...
      setActiveDesign((current) =>
        current ? { ...current, run_id: run.id } : current,
      );
      setDesigns((current) =>
        current.map((item) =>
          item.id === activeDesign?.id ? { ...item, run_id: run.id } : item,
        ),
      );
  ...
  useEffect(() => {
    if (parsedDoc) {
      ...
      const keyPoints = parsedDoc.knowledge_points
        .filter((point) => point.is_key_point)
        .map((point) => point.title);
      const selectedPoints = importedOutline
        ? parsedDoc.knowledge_points.map((point) => point.title)
        : keyPoints.length
          ? keyPoints
          : parsedDoc.knowledge_points.slice(0, 3).map((point) => point.title);
      setScope({
        selected_point_titles: selectedPoints,
        estimated_minutes: 45,
        ppt_slide_count: 45,
        depth: "standard",
      });
      setPagesCustomized(false);
      setScopeConfirmed(
        importedOutline && parsedDoc.knowledge_points.length > 0,
      );
  ...
  useEffect(() => {
    ...
    setScope((current) => ({
      ...current,
      selected_point_titles: current.selected_point_titles.filter((title) =>
        available.has(title),
      ),
    }));
  }, [editedPoints]);
  ...
  const liveNode = useMemo(() => {
    const latestStarted = [...events]
      .reverse()
      .find((event) => event.event_type === "node.started" && event.node)?.node;
    if (
  ...
  const completedNodes = new Set(
    events
      .filter((event) =>
        ["node.completed", "review.completed"].includes(event.event_type),
      )
      .map((event) => event.node),
  );
  const displayRunStatus =
    selectedRun?.status === "queued" &&
    events.some((event) => event.event_type === "node.started")
      ? "running"
      : selectedRun?.status;
  ...
  const processLogEntries = useMemo(() => {
    const logs: Array<{
      ...
      time: string;
      completed: boolean;
      kind: "start" | "heartbeat" | "done";
    }> = [];
    const nodeLabels: Record<string, string> = {
      content_analysis: "内容剖析",
    ...
    events.forEach((event) => {
      if (event.event_type === "node.started" && event.node) {
        logs.push({
          ...
          node: event.node,
          message:
            event.message || `开始${nodeLabels[event.node] || event.node}`,
        ...
        if (event.node)
          logs.push({
            sequence: event.sequence,
            node: event.node,
            message: `完成${nodeLabels[event.node] || event.node}`,
            time: new Date(event.created_at).toLocaleTimeString("zh-CN", {
              hour12: false,
            }),
            completed: true,
        ...
        logs.push({
          sequence: event.sequence,
          node: event.node,
          message: event.message || "智能体仍在处理当前任务",
          time: new Date(event.created_at).toLocaleTimeString("zh-CN", {
            hour12: false,
          }),
          completed: false,
  ...
  const exportReport = async (
    format: "md" | "pdf",
    variant: "teacher" | "student" = "teacher",
  ) => {
    if (!selectedRun) return;
    setError(null);
    try {
      const response = await fetch(
        workflowApi.reportUrl(selectedRun.id, format, variant),
      );
      if (!response.ok) {
        ...
        throw new Error(
          payload?.detail || `导出失败（HTTP ${response.status}）`,
        );
      }
  ...
  const workspaceTitle =
    ...
    parsedDoc?.course_name ||
    activeDesign?.title ||
    "请选择或创建课程";
  const selectedClassroomRun =
    selectedRun?.teaching_data?.workflow_version === "classroom_v2"
      ? selectedRun
      : null;
  const selectedRunScope = selectedClassroomRun?.teaching_data?.scope;
  const selectedRunPoints = selectedRunScope?.selected_point_titles || [];
  const workspaceSubtitle = selectedRun
    ? teaching.document_name || "课程材料"
    : parsedDoc
      ? parsedDoc.file_name
      : "从资料整理阶段确认来源后，再预览范围并启动多智能体教学设计。";
  if (loading)
    return (
  ...
  return (
    <div
      className={`app-shell ${appStage === "overview" ? "overview-shell" : ""}`}
    >
      <header className="topbar">
        <div className="brand">
          ...
          <div>
            <strong>课程教学智能体平台</strong>
            <span>LangGraph Teaching Studio</span>
          </div>
        </div>
        <div className="topbar-meta">
          ...
          <ModelQuickSwitcher
            ...
            onChanged={(model) => {
              setModelSettings(model);
              setModelLabel(
                model.provider === "mock" ? "本地演示模型" : model.model,
              );
              setModelRefreshKey((value) => value + 1);
            }}
          ...
          <button
            ...
            title={loading ? "正在刷新教学会话…" : "刷新教学会话"}
            aria-label="刷新教学会话"
            aria-busy={loading}
            disabled={loading}
          >
            {/* 刷新涉及多次网络请求，没有旋转反馈时教师会重复点击 */}
      ...
      {appStage === "overview" ? (
        ...
        <DataHubWorkspace
          ...
          onOpenArchive={(archiveId) => {
            void loadArchive(archiveId);
            navigateMaterials("preparation");
          }}
          ...
          onMaterialUnitImported={() =>
            setMaterialUnitRefreshKey((value) => value + 1)
          }
        />
      ) : appStage === "materials" ? (
        materialSubpage === "units" ? (
          <MaterialUnitWorkspace
            ...
            onCourseDesignCreated={async (design) => {
              ...
              setParsedDoc({
                ...
                knowledge_points: design.content.knowledge_points.map(
                  (point) => ({
                    title: point,
                    chapter: design.content.chapter,
                    is_key_point: design.content.key_points.includes(point),
                    difficulty_level: design.content.difficult_points.includes(
                      point,
                    )
                      ? "困难"
                      : "标准",
              ...
              setContext(
                `已导入资料单元知识大纲 v${design.knowledge_outline_version || 1}。${design.content.session_label || ""}`.trim(),
              );
              setSelectedRunId(null);
              setWorkspaceView("material");
              setProjectPanelOpen(true);
          ...
          <main className="stage-page materials-stage">
            <header className="stage-page-header">
              <div>
                ...
                <h1>确认教学使用范围</h1>
                <p>
                  原有的使用资料与主材料选择保持不变；资料单元中的分析结果可作为上游依据。
                </p>
              </div>
              <button
                type="button"
                className="secondary-button"
                onClick={() => navigateMaterials("units")}
              >
                <FolderOpen size={15} />
                返回资料单元
              </button>
            </header>
            <CourseArchiveWorkspace
              summaries={archives}
              archive={selectedArchive}
              loading={archiveLoading}
              importing={archiveImporting}
              preparing={archivePreparing}
              prepareProgress={{
                done: archiveImportCounts.content,
                total: archiveImportCounts.total,
              }}
              importStatus={archiveImportStatus}
              onImport={() => navigateStage("hub")}
              onSelectArchive={(archiveId) => void loadArchive(archiveId)}
              onDeleteArchive={(archiveId) => void removeArchive(archiveId)}
              onPrepare={(input) => void prepareArchive(input)}
            />
          </main>
        )
        ...
        <ExportWorkspace
          ...
          onUpdated={(design) => {
            ...
            setDesigns((current) =>
              current.map((item) =>
                item.id === design.id ? designSummary(design) : item,
              ),
            );
          }}
        ...
        <main
          className={`workspace workspace-view-${workspaceView} ${projectPanelOpen ? "project-open" : "project-closed"} ${detailPanelOpen && String(workspaceView) !== "process" ? "detail-open" : "detail-closed"}`}
        >
          {projectPanelOpen && (
            <aside className="left-panel">
              <div className="panel-heading">
                <div>
                  <span className="eyebrow">教学项目</span>
                  <h1>课程设计</h1>
                </div>
                <button
                  className="icon-button"
              ...
              <section
                className={`session-form${selectedClassroomRun ? " selected-run-config" : ""}`}
                aria-label={
                  selectedClassroomRun
                    ? "当前教学设计配置"
                    : "创建教学设计会话"
                }
              >
                {selectedClassroomRun && (
                  <div className="selected-run-summary">
                    <header>
                      <span className="selected-run-summary-icon">
                        <FileCheck2 size={16} />
                      </span>
                      <div>
                        <strong title={selectedClassroomRun.objective}>
                          {selectedClassroomRun.objective}
                        </strong>
                        <small>本次运行配置 · 启动后保持不变</small>
                      </div>
                    </header>
                    <dl>
                      <div>
                        <dt>资料</dt>
                        <dd>
                          {selectedClassroomRun.teaching_data.document_name ||
                            "课程资料"}
                        </dd>
                      </div>
                      <div>
                        <dt>课时</dt>
                        <dd>{selectedRunScope?.estimated_minutes || 45} 分钟</dd>
                      </div>
                      <div>
                        <dt>PPT</dt>
                        <dd>
                          {selectedRunScope?.ppt_slide_count ||
                            selectedRunScope?.estimated_minutes ||
                            45}{" "}
                          页
                        </dd>
                      </div>
                      <div>
                        <dt>深度</dt>
                        <dd>
                          {selectedRunScope?.depth === "overview"
                            ? "概览"
                            : selectedRunScope?.depth === "deep"
                              ? "深入"
                              : "标准"}
                        </dd>
                      </div>
                      <div>
                        <dt>演练</dt>
                        <dd>
                          {selectedClassroomRun.teaching_data.max_iterations || 1} 轮
                        </dd>
                      </div>
                    </dl>
                    <section aria-label="本次教学知识点">
                      <span>
                        教学范围 <b>{selectedRunPoints.length}</b>
                      </span>
                      <div>
                        {selectedRunPoints.map((point) => (
                          <span key={point}>{point}</span>
                        ))}
                      </div>
                    </section>
                    <p>
                      PPT 内容请在“PPT 审阅”中逐页修改；新建课程请点击上方加号。
                    </p>
                    <button
                      type="button"
                      className="run-config-upload"
                ...
                {!parsedDoc ? (
                  ...
                  >
                    ...
                    <strong>
                      {uploading
                        ? `${uploadStage} · ${uploadElapsedText}`
                        : "上传课程材料"}
                    </strong>
                    <small>
                      {uploading
                        ? uploadFileName
                        : "PDF、DOCX、PPTX、Markdown、TXT · 最大 30 MB"}
                    </small>
                  </label>
                ) : (
                  <div className="uploaded-file">
                    <FileCheck2 size={19} />
                    <div>
                      <strong>{parsedDoc.file_name}</strong>
                      <small>
                        {parsedDoc.character_count.toLocaleString()} 字 ·{" "}
                        {parsedDoc.knowledge_points.length} 个候选知识点
                      </small>
                    </div>
                    <button
                      type="button"
                ...
                <button
                  ...
                  onClick={() => navigateStage("materials")}
                ...
                >
                  ...
                  {archives.length > 0 && !archiveImporting && (
                    <small>{archives.length}</small>
                  )}
                </button>
                {parsedDoc && (
                  <details className="knowledge-editor-disclosure">
                    <summary>
                      <span>
                        <ListChecks size={14} />
                        编辑候选知识点
                      </span>
                      <small>{editedPoints.length} 项</small>
                      <ChevronDown size={14} />
                    </summary>
                ...
                {parsedDoc && (
                  <TeachingScopePlanner
                    ...
                    onChange={(nextScope) => {
                      setScope((current) => ({
                        ...nextScope,
                        ppt_slide_count: pagesCustomized
                          ? current.ppt_slide_count
                          : nextScope.estimated_minutes,
                      }));
                      setScopeConfirmed(false);
                    }}
                  />
                ...
                <label>
                  ...
                  <textarea
                    ...
                    rows={2}
                    placeholder="可选：学情、课时或教学侧重点"
                  />
                </label>
                <div className="ppt-page-setting">
                  <div>
                    <span>PPT 页数</span>
                    <small>默认 1 分钟 / 页，可修改</small>
                  </div>
                  <div className="ppt-page-stepper">
                    <button
                      type="button"
                      onClick={() => {
                        setPagesCustomized(true);
                        setScope((current) => ({
                          ...current,
                          ppt_slide_count: Math.max(
                            1,
                            (current.ppt_slide_count ||
                              current.estimated_minutes) - 1,
                          ),
                        }));
                      }}
                      aria-label="减少一页"
                    ...
                    <input
                      ...
                      min={1}
                      max={120}
                      value={scope.ppt_slide_count || scope.estimated_minutes}
                      onChange={(event) => {
                        setPagesCustomized(true);
                        setScope((current) => ({
                          ...current,
                          ppt_slide_count: Math.min(
                            120,
                  ...
                  <button
                    className="ppt-page-reset"
                    type="button"
                    onClick={() => {
                      setPagesCustomized(false);
                      setScope((current) => ({
                        ...current,
                        ppt_slide_count: current.estimated_minutes,
                      }));
                    }}
                  >
                    恢复建议值
                  ...
                  <div className="segmented">
                    {[1, 2, 3].map((value) => (
                      <button
                        key={value}
                        type="button"
                        className={iterations === value ? "active" : ""}
                ...
                <div className="intervention-setting">
                  <span>
                    我要参与<small>不勾选则全自动运行；勾选后会在对应节点停住等你</small>
                  </span>
                  <label className="check">
                    <input
                      type="checkbox"
                      checked={interventions.after_design}
                      onChange={(event) =>
                        setInterventions((value) => ({
                          ...value,
                          after_design: event.target.checked,
                        }))
                      }
                    />
                    <Hand size={13} />
                  ...
                  <label className="check">
                    <input
                      type="checkbox"
                      checked={interventions.after_question}
                      onChange={(event) =>
                        setInterventions((value) => ({
                          ...value,
                          after_question: event.target.checked,
                        }))
                      }
                    />
                    <Hand size={13} />
                ...
                {parsedDoc && (
                  <div className="duration-hint">
                    <Clock3 size={12} />
                    {modelLabel === "本地演示模型"
                      ? "本地演示模型约 10 秒完成；真实模型 1 轮约 2-3 分钟，可随时停止"
                      : "真实模型 1 轮约 2-3 分钟，可随时停止"}
                  </div>
                )}
                <button
                  className="primary-button"
                  ...
                  disabled={
                    ...
                    submitting ||
                    title.trim().length < 2 ||
                    scope.selected_point_titles.length === 0 ||
                    !scopeConfirmed
                  }
                >
                  {submitting ? (
          ...
          <section className="teaching-panel">
            <div
              className={`teaching-top ${String(workspaceView) === "process" ? "process-compact" : ""}`}
            >
              <header className="teaching-header">
                <div>
                  <span className="eyebrow">教师教学工作区</span>
                  ...
                  {selectedRun && (
                    <TeachingProgress
                      ...
                      currentIteration={teaching.current_iteration}
                    />
                  )}
                </div>
                {selectedRun && displayRunStatus && (
                  <div className="run-controls">
                    <span className={`status-badge status-${displayRunStatus}`}>
                      {displayRunStatus === "running" && (
                        <Loader2 className="spin" size={13} />
                      )}
                      {statusLabel(displayRunStatus)}
                    </span>
                    {selectedRun.final_output && (
                      <button
                        className="secondary-button"
                        type="button"
                        onClick={() => navigateExport("lesson")}
                        title="打开当前教案定稿"
                      >
                        <FileDown size={15} />
                        编辑与导出
              ...
              {activeDesign && (
                <div className="design-source-bar">
                  <div>
                    <Link2 size={14} />
                    <span>
                      当前课程设计引用{" "}
                      <strong>{activeDesign.source_references.length}</strong>{" "}
                      条上游数据
                    </span>
                    <small>
                      第 {activeDesign.version} 版 ·{" "}
                      {activeDesign.run_id
                        ? "已关联生成会话"
                        : "待关联生成会话"}
                    </small>
              ...
              >
                <button
                  id="workspace-tab-material"
                  type="button"
                  role="tab"
                  aria-selected={workspaceView === "material"}
                  aria-controls="workspace-panel-material"
                  className={workspaceView === "material" ? "active" : ""}
                ...
                <button
                  id="workspace-tab-process"
                  type="button"
                  role="tab"
                  aria-selected={String(workspaceView) === "process"}
                  aria-controls="workspace-panel-process"
                  className={
                    String(workspaceView) === "process" ? "active" : ""
                  }
                ...
                <button
                  id="workspace-tab-result"
                  type="button"
                  role="tab"
                  aria-selected={workspaceView === "result"}
                  aria-controls="workspace-panel-result"
                  className={workspaceView === "result" ? "active" : ""}
                  onClick={() => setWorkspaceView("result")}
                  disabled={!selectedRun?.final_output}
                >
                  <FileDown size={15} />
                  <span>教学成果</span>
                  <small>预览、编辑与导出</small>
                </button>
              </nav>
            </div>
            ...
            {workspaceView === "material" ? (
              ...
              >
                {previewDocument ? (
                  <DocumentPreviewWorkspace {...previewDocument} />
                ) : activeDesign ? (
                  <div className="empty-state tall">
                    <FileCheck2 size={22} />
                    <strong>材料预览正在加载</strong>
                    <span>
                      已导入知识大纲 v
                      {activeDesign.knowledge_outline_version || 1}，正在准备{" "}
                      {activeDesign.material_ids.length} 份已解析材料的正文预览…
                    </span>
                  </div>
                ) : (
              ...
              <div
                id="workspace-panel-result"
                role="tabpanel"
                aria-labelledby="workspace-tab-result"
                className="workspace-tabpanel full-result-view"
              >
                <ResultPanel
                  run={selectedRun}
              ...
              <div
                id="workspace-panel-process"
                role="tabpanel"
                aria-labelledby="workspace-tab-process"
                className="workspace-tabpanel process-view"
              >
                <ClassroomWorkspace
                  key={selectedRun?.id ?? "classroom-empty"}
                  run={selectedRun}
                />
              </div>
            )}
          ...
          {String(workspaceView) !== "process" && detailPanelOpen && (
            <aside className="detail-panel">
              <div
                className="detail-tabs detail-tabs-three"
                role="tablist"
                aria-label="课程洞察"
              >
                <button
                  ...
                  className={detailTab === "analysis" ? "active" : ""}
                ...
                <button
                  id="detail-tab-framework"
                  type="button"
                  role="tab"
                  aria-selected={detailTab === "framework"}
                  aria-controls="detail-tabpanel"
                  className={detailTab === "framework" ? "active" : ""}
                ...
                <button
                  ...
                  aria-controls="detail-tabpanel"
                  className={detailTab === "review" ? "active" : ""}
              ...
              >
                {detailTab === "analysis" && <AnalysisPanel data={teaching} />}
                {detailTab === "framework" && (
                  <FrameworkPanel data={teaching} />
                )}
                {detailTab === "review" && (
                  <ReviewPanel run={selectedRun} events={events} />
                )}
              </div>
            </aside>
          )}
...
function AnalysisPanel({ data }: { data: TeachingData }) {
  ...
  if (!analysis)
    return (
      <div className="empty-state tall">
        <Layers3 size={21} />
        <strong>尚未完成内容剖析</strong>
        <span>教师智能体将从上传材料中识别知识结构。</span>
      </div>
    );
  const copyText = [
    analysis.summary,
    `教学重点：${analysis.key_points?.join("；") || ""}`,
    `学习难点：${analysis.difficult_points?.join("；") || ""}`,
    `常见误区：${analysis.learner_misconceptions?.join("；") || ""}`,
  ].join("\n\n");
  return (
    <div className="analysis-view">
      <div className="panel-copy-row">
        <strong>课程内容剖析</strong>
        <CopyTextButton text={copyText} label="复制内容剖析" />
      </div>
      <p className="analysis-summary">{analysis.summary}</p>
      <DetailList title="教学重点" items={analysis.key_points} tone="green" />
      <DetailList
        title="学习难点"
        items={analysis.difficult_points}
        tone="amber"
      />
      <DetailList
        title="常见误区"
        items={analysis.learner_misconceptions}
        tone="red"
      />
      <div className="knowledge-cloud">
        {data.knowledge_points?.map((point) => (
          <span key={point.title} className={point.is_key_point ? "key" : ""}>
            {point.title}
          </span>
        ))}
      </div>
...
function DetailList({
  ...
  return (
    <section className={`detail-section tone-${tone}`}>
      <h3>
        <span>{title}</span>
        <span className="detail-section-tools">
          <CopyTextButton text={items.join("\n")} label={`复制${title}`} />
...
const exerciseLevelNames: Record<string, string> = {
  low: "基础",
  medium: "进阶",
  high: "拓展",
};
...
function ExerciseCard({
  ...
  return (
    <div className="exercise-card">
      <header>
        <span className="exercise-no">{index + 1}</span>
        <span className={`exercise-level level-${exercise.level}`}>
          {exerciseLevelNames[exercise.level]}
        </span>
      </header>
...
function FrameworkPanel({ data }: { data: TeachingData }) {
  ...
  if (!framework)
    return (
      <div className="empty-state tall">
        <FileText size={21} />
        <strong>尚未形成教学环节</strong>
        <span>内容剖析完成后将生成教学框架。</span>
      </div>
    );
  const copyText = [
  ...
  return (
    <div className="framework-view">
      <div className="panel-copy-row">
        <strong>当前教学设计</strong>
        <CopyTextButton text={copyText} label="复制教学设计" />
      </div>
      <DetailList
        title="学习目标"
        items={framework.learning_objectives}
        tone="green"
      />
      ...
      {framework.exercises?.length ? (
        <section className="detail-section">
          <h3>
            <span>课堂练习</span>
            <span className="detail-section-tools">
              <CopyTextButton
                text={framework.exercises
                  .map((item) => `${item.question}\n参考：${item.answer}`)
                  .join("\n\n")}
                label="复制课堂练习"
              />
              <small>{framework.exercises.length}</small>
            </span>
          </h3>
          <div className="exercise-list">
            {framework.exercises.map((exercise, index) => (
              <ExerciseCard
      ...
      <DetailList title="评价方式" items={framework.assessment} tone="amber" />
    </div>
  );
}
function ReviewPanel({
  ...
  const eventReview = [...events]
    .reverse()
    .find((event) => event.payload.review)?.payload.review as
    WorkflowRun["review"] | undefined;
  const review = run?.review || eventReview;
  if (!review)
    return (
      <div className="empty-state tall">
        <ShieldCheck size={21} />
        <strong>等待督导评价</strong>
        <span>完成首轮讲授和答疑后生成。</span>
      </div>
    );
  return (
    <div className="review-view">
      ...
      <DetailList title="教学亮点" items={review.strengths} tone="green" />
      <DetailList title="主要不足" items={review.weaknesses} tone="amber" />
      <DetailList title="改进建议" items={review.suggestions} tone="red" />
      {review.next_focus && (
        <div className="next-focus">
          <MessageCircleQuestion size={16} />
          <div>
            <strong>下一轮教学重点</strong>
            <p>{review.next_focus}</p>
          </div>
        </div>
      )}
      {review.iteration_prompt && (
        <section className="reusable-prompt">
          <header>
            <strong>可复用优化提示词</strong>
            <CopyTextButton
              text={review.iteration_prompt}
              label="复制可复用优化提示词"
            />
          </header>
          <p>{review.iteration_prompt}</p>
        </section>
      )}
    </div>
  );
...
function ResultPanel({
  ...
  if (!report)
    return (
      <div className="empty-state tall">
        <FileDown size={21} />
        <strong>尚未生成教学成果</strong>
        <span>
          全部教学轮次完成后，这里将汇总课程剖析、学习目标与督导结论。
        </span>
      </div>
    );
  return (
    <TeacherMaterialsWorkspace
      run={run}
      onExport={onExport}
      onError={onError}
    />
